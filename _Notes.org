...
method
------
Read number of points, allocate pts array.
Read and store points into pts array.

Create empty oshapenums_by_physreg.
Create empty oshape_ref_tris.
Create empty side_incls (size hint to # fes * 3/2).
Read number of elements, allocate fes array.
num_nb_sides = 0
num_b_sides = 0
Read elements, for each element:
  ...
  Create new RefTri and oshape # (= length(oshape_ref_tris) + 1) if phys region attribute is not yet in oshapes_by_physprop (in which case add it)
...
For each entry in side_incls
  if value has two fe inclusions
    ++nbsidenum
    create NBSideInclusions, store at nb_side_incls[nbsidenum].
