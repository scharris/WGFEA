...
logical_dims(mesh::RectMesh) = mesh.mesh_ldims
...
function exportAsGmshGeo(ios::IO,  mesh::RectMesh; targetGmshElSize::R = 1., asTriangles::Bool=false)
  if mesh.space_dim != 2 
  ...
  const get_linenum = let next_linenum = let next = int64(1); () -> (next += 1) - 1 end;
    function(ptnum_1::PointNum, ptnum_2::PointNum)
  ...
  for fe=Mesh.fenum(1):mesh.num_fes
    ...
    # lower left vertex
    const ll_pointnum = get_pointnum((fe_mcoords[1], fe_mcoords[2]))

    # lower right vertex
    const lr_pointnum = get_pointnum((fe_mcoords[1]+1, fe_mcoords[2]))
    ...
    # upper right vertex
    const ur_pointnum = get_pointnum((fe_mcoords[1]+1, fe_mcoords[2]+1))
    ...
    # upper left vertex
    const ul_pointnum = get_pointnum((fe_mcoords[1], fe_mcoords[2]+1))
    ...
    if !asTriangles
      ...
      const ll_ur_linenum = get_linenum(ll_pointnum, ur_pointnum)
      if ll_ur_linenum > 0 # new line: declare in Gmsh
        @printf(ios, "Line(%d) = {%u, %u};\n", ll_ur_linenum, ll_pointnum, ur_pointnum)
      end

      # line loop representing the lower triangle
      const lower_tri_num = uint64(2*fe-1)
      @printf(ios, "Line Loop(%u) = {%d, %d, %d};\n", lower_tri_num, ll_lr_linenum, lr_ur_linenum, -ll_ur_linenum)
      # surface representing the lower triangle
      @printf(ios, "Plane Surface(%u) = {%u};\n", lower_tri_num, lower_tri_num)
      
      # line loop representing the upper triangle
      const upper_tri_num = uint64(2*fe)
      @printf(ios, "Line Loop(%u) = {%d, %d, %d};\n", upper_tri_num, ll_ur_linenum, ur_ul_linenum, ul_ll_linenum)
      # surface representing the upper triangle
      @printf(ios, "Plane Surface(%u) = {%u};\n", upper_tri_num, upper_tri_num)
      
      # Tag both triangles with physical region tag with value of this finite element number.
      @printf(ios, "Physical Line(%u) = {%d, %d, %d};\n", fe, ll_lr_linenum, lr_ur_linenum, -ll_ur_linenum)
      @printf(ios, "Physical Line(%u) = {%d, %d, %d};\n", fe, ll_ur_linenum, ur_ul_linenum, ul_ll_linenum)
    end
  end
